...
metadata2 <- metadata %>%
  ...
  mutate(SLP_score = case_when(as.numeric(SLP)/30 <= 3 ~ "Refractario",
                               as.numeric(SLP)/30 > 3 & as.numeric(SLP)/30 <= 12 ~ "Respondedor",
                              as.numeric(SLP)/30 > 12 & as.numeric(SLP)/30 <= 24 ~ "Largo",
...
dds_gene_SLP <- DESeqDataSetFromMatrix(countData = round(table_trim_SG_SLP_Resp), 
                                   colData = meta_SG_SLP_Resp, 
                                   design = ~ SLP_score)
dds_gene_SLP_lrt <- DESeq(dds_gene_SLP, test = "LRT", reduce = ~ 1)
...
```{r}
res_LRT <- results(dds_gene_SLP_lrt) %>% as.data.frame() %>% as_tibble(rownames="gene")
```
...
#### Refractario vs Respondedor

```{r}

SLP_refractario_respondedor <- results(dds_gene_SG_SLP_Resp, contrast = c("SLP_score","Refractario","Respondedor"))

SLP_refractario_respondedor <- SLP_refractario_respondedor %>% 
  as_tibble(rownames = "ensembl_gene_id") %>%
    dplyr::rename(fdr = padj) %>% 
  mutate(bonferroni = p.adjust(pvalue, method = "bonferroni")) %>% 
  mutate(homl = p.adjust(pvalue, method = "holm")) %>% 
...
SLP_refractario_respondedor %>% filter(fdr < 0.05)%>% DT::datatable(extensions = "Buttons", 
            options = list(paging = TRUE,
                           scrollX=TRUE, 
                           searching = TRUE,
                           ordering = TRUE,
...
#### Respondedor vs Largo

```{r}

SLP_responderdor_Largo <- results(dds_gene_SG_SLP_Resp, contrast = c("SLP_score","Respondedor","Largo"))

SLP_responderdor_Largo <- SLP_responderdor_Largo %>% 
  as_tibble(rownames = "ensembl_gene_id") %>%
    dplyr::rename(fdr = padj) %>% 
  mutate(bonferroni = p.adjust(pvalue, method = "bonferroni")) %>% 
  mutate(homl = p.adjust(pvalue, method = "holm")) %>% 
...
SLP_responderdor_Largo %>% 
  ...
  geom_text_repel(size = 1.5)+
  scale_color_d3() +
  theme_light() + labs(title = "SLP_responderdor_Largo")

SLP_responderdor_Largo %>% filter(fdr < 0.05)%>% DT::datatable(extensions = "Buttons", 
            options = list(paging = TRUE,
                           scrollX=TRUE, 
                           searching = TRUE,
                           ordering = TRUE,
...
#### Respondedor vs MuyLargo

```{r}

SLP_responderdor_MuyLargo <- results(dds_gene_SG_SLP_Resp, contrast = c("SLP_score","Respondedor","MuyLargo"))

SLP_responderdor_MuyLargo <- SLP_responderdor_MuyLargo %>% 
  as_tibble(rownames = "ensembl_gene_id") %>%
    dplyr::rename(fdr = padj) %>% 
  mutate(bonferroni = p.adjust(pvalue, method = "bonferroni")) %>% 
  mutate(homl = p.adjust(pvalue, method = "holm")) %>% 
...
SLP_responderdor_MuyLargo %>% 
  ...
  geom_text_repel(size = 1.5)+
  scale_color_d3() +
  theme_light() + labs(title = "SLP_responderdor_MuyLargo")

SLP_responderdor_MuyLargo %>% filter(fdr < 0.05)%>% DT::datatable(extensions = "Buttons", 
            options = list(paging = TRUE,
                           scrollX=TRUE, 
                           searching = TRUE,
                           ordering = TRUE,
...
### Clustering de RNA-Seq

```{r}
library(vegan)
library(mclust)

cuentas <- counts(dds_gene_heng, normalized = T)

distancias <- vegdist(cuentas %>% t()) %>% as.matrix()


cl <- Mclust(distancias)


um <- uwot::umap(distancias)

clustering_metadata <- cl$classification %>% 
  as_tibble(rownames = "Sample") %>%
  dplyr::rename(Cluster = value) %>% 
  mutate(Cluster = as.factor(Cluster)) %>% 
  mutate(Sample = str_remove(Sample,"X")) %>% 
  mutate(Sample = str_remove(Sample,regex("_S.*"))) %>% 
  mutate(Sample = str_replace_all(Sample,regex("\\."),"-")) %>% 
  separate(Sample,c("Centro","Pat"), sep = "-",remove = F) %>% 
  mutate(Centro = as.numeric(Centro)) %>% 
  mutate(Pat = as.numeric(Pat)) %>% 
  unite(ID,c(Centro,Pat),sep = "-") %>% 
  inner_join(metadata2) %>% 
...
Test de asociación con la distribución de las muestras segun su perfil de expresión

```{r}
adonis2(distancias ~ SLP_score + SG_score + Resp + heng, data = clustering_metadata %>% column_to_rownames("Sample"))
```

```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = Cluster)) + 
  geom_point() + labs(title = "Cluster distribution")
```
...
```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = SLP_score)) + 
  geom_point() + labs(title = "SLP distribution")
p_slp <- clustering_metadata %>% 
  freq_table(Cluster,SLP_score) %>% 
  ggbarstats(SLP_score,Cluster,counts = n)
p_slp
extract_stats(p_slp)
```
...
```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = SG_score)) + 
  geom_point() + labs(title = "SG distribution")
p_sg <- clustering_metadata %>% 
  freq_table(Cluster,SG_score) %>% 
  ggbarstats(SG_score,Cluster,counts = n)
p_sg
extract_stats(p_sg)
```
...
```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = Resp)) + 
  geom_point() + labs(title = "Respuesta distribution")
p_Resp <- clustering_metadata %>% 
  filter(Resp != "NE") %>%  
  freq_table(Cluster,Resp) %>% 
  ggbarstats(Resp,Cluster,counts = n)
p_Resp
extract_stats(p_Resp)
```

```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = heng)) + 
  geom_point() + labs(title = "HENG distribution")

p_heng <- clustering_metadata %>% 
  freq_table(Cluster,heng) %>% 
  ggbarstats(heng,Cluster,counts = n)
p_Resp
extract_stats(p_heng)
```

```{r}
clustering_metadata %>% 
  ggplot(aes(x = V1, y = V2, color = estadio)) + 
  geom_point() + labs(title = "Estadio distribution")

p_estadio <- clustering_metadata %>% 
  freq_table(Cluster,estadio) %>% 
  ggbarstats(estadio,Cluster,counts = n)
p_estadio
extract_stats(p_estadio)
...
```{r}
table_trim_cluster <- count_trim %>% 
  inner_join(clustering_metadata %>% dplyr::select(ID,Cluster)) %>% 
  drop_na() %>% 
  dplyr::select(gene_id,Sample, counts) %>% 
  pivot_wider(names_from = Sample, values_from = counts) %>% 
  as_data_frame() %>% 
 ...
 dds_gene_cluster <- DESeqDataSetFromMatrix(countData = round(table_trim_heng), 
                                  colData = clustering_metadata %>% column_to_rownames("Sample"), 
                                 design = ~ Cluster)

 dds_gene_cluster <- DESeq(dds_gene_cluster, test = "LRT", reduced = ~ 1)

```

```{r fig.width= 14, fig.height=14}
resultsNames(dds_gene_cluster)


sig_genes <- lfcShrink(dds_gene_cluster, "Cluster_3_vs_1", format = "DataFrame") %>% 
  as.data.frame() %>% 
  rownames_to_column("ensembl_gene_id") %>% 
  filter(padj < 0.05)

sig_genes %>% inner_join(ttg) %>%  DT::datatable(extensions = "Buttons", 
            options = list(paging = TRUE,
                           scrollX=TRUE, 
                           searching = TRUE,
                           ordering = TRUE,
...
counts(dds_gene_cluster, normalized = T) %>% 
  as_tibble(rownames = "ensembl_gene_id") %>% 
  pivot_longer(names_to = "Sample", values_to = "Expression", -ensembl_gene_id) %>% 
  semi_join(sig_genes) %>% 
  inner_join(clustering_metadata) %>% 
  inner_join(ttg) %>% 
  ggplot(aes(x = Cluster, y = Expression, color = Cluster)) + 
  geom_boxplot() + 
  geom_jitter() +
...
```{r}
ggbetweenstats(DNA_TMB_test, x = heng, y = Mutations,type = "robust")
```

### SG

```{r}
  ggbetweenstats(DNA_TMB_test, x =SG_score , y = Mutations,type = "robust")
```

### SLP

```{r}
ggbetweenstats(DNA_TMB_test, x = SLP_score, y = Mutations,type = "robust")
```
